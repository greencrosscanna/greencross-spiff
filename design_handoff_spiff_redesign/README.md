...
## Fidelity
**High-fidelity.** Colours, type, spacing, radii, and states in the mockups are final and should be matched exactly. Every value comes from the existing gx-theme palette — nothing new was invented. Where a value below is given as a hex, it is because the mockups had to inline it; in the codebase prefer the equivalent `gx-theme.css` custom property.
...
Store colours come from the **GX Core store registry** (`gx_core.gs` → `GX_STORE_SEED`, mirrored in `gx-stores.js`). Never hardcode them per screen; read them from the registry so new stores inherit correctly. Values in the mockups:
...
- Severity is expressed as a **3px left border** on an otherwise normal card (gold `#d4a847` for attention, red `#ef4444` for blocked), plus a matching uppercase micro-label in the same colour.
...
**Sub-nav / filter bar** — a second bar directly under the header, background `#121715`, bottom border `#232a27`, padding `10px 20px`, `flex-wrap:wrap; gap:9px`. Starts with an uppercase 11px/700 `#5e6864` label naming the bar's job ("Modelling", "Filter", …), then search input, scope pills, and store pills. Inputs: background `#1a221f`, border `#232a27`, radius `8px`, `padding:6px 10px`, 12.5px, `color-scheme:dark`.

**Stat strip** — a row of figures directly under the sub-nav: big Montserrat number over a 10px uppercase muted label. Figures that carry a judgement take the accent green or the severity colour; neutral figures stay `#e6ece9`.
...
The working modelling view. Sub-nav label is "Modelling" and carries a program `<select>` (max-width 240px).
- **One merged store table** — all six stores in a single table, not one card per store. Columns are right-aligned tabular numbers; the store name cell carries the registry colour as a leading dot.
...
Mid-pull state, showing all four store-card conditions at once — this is the reference for loading behaviour.
- Store cards, three across: registry-colour dot + store name + unit count; a 5px `#1a221f` track with a `999px` fill in the store colour; a footer line reading "N of M hit" (green when complete) and the per-store average.
...
The lookup surface. Sub-nav is search + month scope. Rows are dense (12.5px, tabular numerics), grouped under sticky month labels, with the outcome figure right-aligned and coloured only when it beat or missed target.
...
- The eighteen inputs are labelled rows inside those groups; only program identity keeps the field-card treatment.
...
The budtender phone view, 390px wide, radius `26px`, padding `20px 16px 16px`. Big single figure ("what you've earned"), then the active programs as compact rows. Optimised for glancing at on the floor: nothing below 44px is tappable, and the earned figure is the largest thing on screen.
...
- No other images. Store colours are data, from the registry.
